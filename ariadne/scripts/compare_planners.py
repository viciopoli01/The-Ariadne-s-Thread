...
def plan_with_random_map_global() -> Tuple[bool, bool, bool, float, float, float, float, float, float]:
    ...
    rrt_course = rrt_planner.plan(current_pose, local_goal_pose, list(obstacles[:, :2]), list(obstacles[:, 2]), show_animation=False,
                                  map_bounds=[int(-map_width / 2), int(-map_height / 2), int(map_width / 2), int(map_height / 2)],
...
def plan_with_random_map_with_heli() -> Tuple[bool, bool, bool, float, float, float, float, float, float]:
    ...
    while np.any(local_goal_pose != global_goal_pose):
        ...
        if not astar_failed:
            t = time.time()
            astar_course = astar_planner.plan(current_pose[:2], local_goal_pose[:2], list(obstacles[:, :2]), list(obstacles[:, 2]), show_animation=True,
...
def main():
    ...
    fail_count_df.to_csv(path_or_buf=f'/home/yuhang/ariadne_ws/src/The-Ariadne-s-Thread/ariadne/scripts/fail_count.csv', index=False)
    results_df.to_csv(path_or_buf=f'/home/yuhang/ariadne_ws/src/The-Ariadne-s-Thread/ariadne/scripts/results.csv', index=False)
